...
pixel_to_address_proc : process(cclk)       --c1_output, r1_output)--only does center of ball
...
block_destroy_proc : process(addr_s) --, c1_output, r1_output
    --vars/signals/constants
    begin    
        --functions only destroys blocks ball passes through
        if (data_in(0) = '1' ) then --checks block of RAM that ball is currently inside, if block exists
            we <= '1';  --RAM write enable
            data_out <= "00000";    --writes blank block to RAM spot
            block_inv_r <= '1';
        elsif block_inv_r = '1' then
            block_inv_r <= '0';
        end if;
        --end previously functioning code
    
end process block_destroy_proc;

ball_direction_invert_proc : process( block_inv_c, block_inv_r, wall_inv_c, wall_inv_r)
begin

end process ball_direction_invert_proc;
